...
- **Scheduler is accounting-only (2026-09-08).** No asm context-switch stub exists, so `sched_tick()` (timer IRQ) NEVER switches CR3/ESP0 itself — it only counts the slice down; handoffs happen at safe points (`sched_yield`, sys_exit, spawn/prepare/reap via `process_switch`). Switching address spaces from a timer IRQ would strand the preempted kernel thread mid-frame. Same reason `process_switch` is handoff-point-only.
