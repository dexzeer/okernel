...
// Readiness (first honest seed): >= 64B from >= 2 source classes.
static void rng_reseed(void) {
    ...
    if (!rng_ready_flag && pool0_n * 32 >= 64) {
        int classes = 0;
        for (int b = 0; b < 4; b++)
            if (pool_src_mask & (1 << b)) classes++;
        if (classes >= 2) rng_ready_flag = 1;
    }
    pools_wipe();
}
...
int rand_hw_init(void) {
    uint32_t a, b, c, d;
    cpuid32(1, &a, &b, &c, &d);
    if (!(c & (1u << 30))) return 0; // no RDRAND: pools only (logged by caller)
    uint8_t hw[32];
    for (int i = 0; i < 8; i++) {
        uint32_t w = 0;
        if (!rdrand32(&w)) return 0;
        hw[4*i] = (uint8_t)(w & 0xff); hw[4*i+1] = (uint8_t)((w >> 8) & 0xff);
        hw[4*i+2] = (uint8_t)((w >> 16) & 0xff); hw[4*i+3] = (uint8_t)(w >> 24);
    }
    rand_stir_src(hw, RAND_SRC_RDRAND);
